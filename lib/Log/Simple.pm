...
sub open {
    ...
    # Assemble mode
    # Always Nonblocking, write only and creat as needed.
...
Log::Simple - A simple, object oriented logfile management library.
...
=head1 SYNOPSIS
  ...
  my $errors = Log::Simple->new( $logfile, sub { return "OOPS: $_[0]\n" } );
  
  $log->message($hash_ref, sub { Dumper @_ });
...
Log::Simple is intended to simplify aspects of managing a logfile.  It uses object oriented interface that is both simple and flexible.  This module will not pollute your namespace, it does not export anything.
